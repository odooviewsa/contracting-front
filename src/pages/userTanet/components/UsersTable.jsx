...
const UsersTable = ({ content }) => {
  const [usersTanets, setUsersTanets] = useState(null);
  ...
  useEffect(() => {
    ...
    async function fetchUsers() {
      try {
        ...
        setUsersTanets(response.data.data);
      } catch (error) {
  ...
  const filteredUsers = usersTanets?.usersGroup?.filter((user) => {
    ...
    <div className="py-8 min-h-[50vh]">
      ...
      <div className="scrollbar overflow-x-outo overflow-y-hidden">
        {paginatedUsers && paginatedUsers.length > 0 ? (
          <table className="border-separate border-spacing-y-3">
            <thead>
              <tr className="text-black">
                {content.table.length > 0 && content?.table?.map((item, key) => (
          ...
          <h1>{usersTanets === null ? "Loading..." : "No Users Found"}</h1>
        )}
      </div>
      {/* Pagination */}
      {paginatedUsers && paginatedUsers.length > 0 && (
