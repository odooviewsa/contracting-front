    ...
    <div className="flex flex-row items-center justify-between  gap-2 sm:gap-8 md:gap-5   flex-nowrap  md:py-8">
      {steps.map((step, index) => (
        step.name.includes('ProgressDahedLine') ? (
          <div key={step.name} className={`w-[120px] border-t-2 border-dashed hidden sm:block ${index <= totalIndex
          ...
          >
            <div className="flex flex-col items-center">
              <div
                className={`w-12 h-12 md:w-16 md:h-16 flex items-center justify-center rounded-full ${index <= totalIndex
              ...
              <span className="mt-2 text-[8px] md:text-sm text-center">{step.name}</span>
            </div>
          </div>
        )
      ))}
