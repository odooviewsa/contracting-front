...
const InputField = ({ label, placeholder, name, type = "text", register, required, errorMessage, options, isClient, defaultValue }) => (
    <div className="mb-4">
        ...
        {type === "select" ? (
            <select
                ...
                className={`w-full px-4 py-2 border rounded-md border-primaryColor focus:outline-none focus:ring-2 focus:ring-blue-500 ${errorMessage ? 'border-red-500' : ''}`}
            >
                {options.map((option) => (
                    <option key={option._id || option} value={option.partnerName || option}>
...
const TextAreaField = ({ label, placeholder, name, register, required, errorMessage }) => (
    <div className="mb-4">
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const namesResponse = await axiosInstance.get('/api/projects/names');
                setManagers(namesResponse.data.groups);
                setTeamMembers(namesResponse.data.groups.map((group) => (
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            toast.success('Project updated successfully!', {
                onClose: () => navigate(`/${user?.companyName}/projects/projectdetails/${id}`)
            });
        } catch (error) {
            console.error(error);
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                        <div>
                            ...
                            <InputField
                                ...
                                register={register}
                                required={true}
                                errorMessage={errors.projectLocation && "Project Location is required"}
                            />
                            <InputField
                                ...
                                register={register}
                                required={true}
                                errorMessage={errors.projectManager && "Project Manager is required"}
                                type="select"
                                options={managers}
